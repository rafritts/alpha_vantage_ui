<script lang="ts">
	...
	async function fetchIncomeStatement() {
		...
		loading = true;
		try {
			const result = await callAlphaVantageFromBrowser('INCOME_STATEMENT', { symbol: s });
			if (!result.ok) {
	...
	function isExpanded(kind: 'annual' | 'quarterly', idx: number) {
		const arr = kind === 'annual' ? expandedAnnualFlags : expandedQuarterlyFlags;
		return !!arr[idx];
	}
...
<section class="space-y-6">
	<PageHeader title="Income Statement" badgeText="Fundamentals" badgeColor="primary" />

	<div class="card bg-base-100 shadow">
		<div class="card-body">
			<div class="flex flex-wrap items-end gap-3">
				<SymbolSearch
					submitButtonText={loading ? 'Loading...' : 'Fetch Income Statement'}
					{loading}
					on:search={() => fetchIncomeStatement()}
				/>
				{#if data}
				...
				{#if tab === 'annual'}
					{#if data.annualReports?.length}
						<div class="mt-3 space-y-3">
							{#each data.annualReports as report, i}
					...
					{/if}
				{:else if data.quarterlyReports?.length}
					<div class="mt-3 space-y-3">
						{#each data.quarterlyReports as report, i}
							{@const splits = splitKeys(report)}
							<div class="overflow-hidden rounded-lg border border-base-300">
