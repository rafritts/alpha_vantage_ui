...
<div class="min-h-dvh flex flex-col bg-base-200">
        ...
        <div class="container mx-auto px-4 flex items-center justify-between">
            ...
            <div class="flex-none flex items-center gap-2">
                <button class="btn btn-secondary" onclick={openApiKeyModal} aria-label="Set API Key">
                    API Key
                    {#if $apiKeyStore}
                        <span class="badge badge-success badge-sm ml-2">Set</span>
                    {:else}
                        <span class="badge badge-ghost badge-sm ml-2">Session-only</span>
                    {/if}
                </button>
    ...
    {#if showApiModal}
        <dialog open class="modal">
            <div class="modal-box">
                <h3 class="font-bold text-lg">Alpha Vantage API Key</h3>
                <p class="py-2 text-sm opacity-80">Your key is stored in sessionStorage and cleared when this tab closes.</p>
                <input
                    type="text"
                ...
                <div class="modal-action">
                    ...
                    <button class="btn btn-primary" onclick={saveApiKey} disabled={!tempKey.trim() && !$apiKeyStore}>Save</button>
                </div>
            </div>
            <button type="button" class="modal-backdrop" onclick={closeApiKeyModal} aria-label="Close"></button>
        </dialog>
    {/if}
