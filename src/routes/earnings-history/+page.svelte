<script lang="ts">
	...
	async function fetchEarnings() {
		...
		loading = true;
		try {
			const result = await callAlphaVantageFromBrowser('EARNINGS', { symbol: s });
			if (!result.ok) {
									...
									<svg viewBox={`0 0 ${vw} ${vh}`} class="w-full max-w-full">
										...
										{#if lastNumber(annual, (d) => toNumber(d.reportedEPS)) !== null}
											<text
												...
												y={pad + 12}
												text-anchor="end"
												class="fill-base-content text-[12px] font-semibold"
												>{formatTick(lastNumber(annual, (d) => toNumber(d.reportedEPS))!)}</text
											>
										{/if}
									...
									<tbody>
										{#each [...annual].reverse() as row}
											<tr>
												...
												<td class="text-right"
													>{toNumber(row.reportedEPS) ??
														'-'}{#if toNumber(row.reportedEPS) !== null}
														USD{/if}</td
												>
											</tr>
										{/each}
								...
								<div class="w-full overflow-x-auto">
									<svg viewBox={`0 0 ${vw} ${vh}`} class="w-full max-w-full">
										...
										<path
											...
											class="fill-none stroke-secondary/70"
										...
										<g>
											...
											<text x={pad + 16} y={pad + 14} class="fill-base-content/80 text-[11px]"
												>Reported EPS</text
											>
											<rect
												x={pad + 130}
												...
												stroke-width="2"
											/>
											<text x={pad + 146} y={pad + 14} class="fill-base-content/80 text-[11px]"
												>Estimated EPS</text
											>
										</g>
										{#if lastNumber(quarterly, (d) => toNumber(d.reportedEPS)) !== null}
											<text
												...
												text-anchor="end"
												class="fill-base-content text-[12px] font-semibold"
												>{formatTick(lastNumber(quarterly, (d) => toNumber(d.reportedEPS))!)}</text
											>
										{/if}
