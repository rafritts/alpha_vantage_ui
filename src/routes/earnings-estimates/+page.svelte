<script lang="ts">
  ...
  async function fetchEstimates() {
    ...
    loading = true;
    try {
      const res = await fetch(`/api/earnings-estimates?symbol=${encodeURIComponent(s)}`);
  ...
  const maxXTicks = 8;
  function xStep(len: number) { return Math.max(1, Math.ceil(len / maxXTicks)); }
  function shortDate(d: string | undefined) { return d ? String(d).slice(0, 7) : ''; }
  function yearOnly(d: string | undefined) { return d ? String(d).slice(0, 4) : ''; }
</script>

<section class="space-y-6">
  <div class="flex items-center gap-3">
    <h1 class="text-3xl font-bold">Alpha Vantage — Earnings Estimates</h1>
                  ...
                  <tbody>
                    {#each [...annual].reverse() as row}
                      <tr>
                        <td>{row.date}</td>
                        <td>{row.horizon}</td>
                        <td class="text-right">{row.epsMean ?? '-'}{#if row.epsMean !== null} USD{/if}</td>
                        <td class="text-right">{row.epsHigh ?? '-'}{#if row.epsHigh !== null} USD{/if}</td>
                        <td class="text-right">{row.epsLow ?? '-'}{#if row.epsLow !== null} USD{/if}</td>
                        <td class="text-right">{row.epsAnalystCount ?? '-'}</td>
                        <td class="text-right">{row.revMean ?? '-'}{#if row.revMean !== null} USD{/if}</td>
                        <td class="text-right">{row.revHigh ?? '-'}{#if row.revHigh !== null} USD{/if}</td>
                        <td class="text-right">{row.revLow ?? '-'}{#if row.revLow !== null} USD{/if}</td>
                      </tr>
                    {/each}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {#each [...quarterly].reverse() as row}
                      <tr>
                        ...
                        <td class="text-right">{row.epsLow ?? '-'}{#if row.epsLow !== null} USD{/if}</td>
                        <td class="text-right">{row.epsAnalystCount ?? '-'}</td>
                        <td class="text-right">{[row.epsAvg7, row.epsAvg30, row.epsAvg60, row.epsAvg90].map(v => v ?? '-').join(' / ')}</td>
                        <td class="text-right">{row.revMean ?? '-'}{#if row.revMean !== null} USD{/if}</td>
                        <td class="text-right">{row.revHigh ?? '-'}{#if row.revHigh !== null} USD{/if}</td>
                        <td class="text-right">{row.revLow ?? '-'}{#if row.revLow !== null} USD{/if}</td>
                        <td class="text-right">{[row.revUp7 ?? '-', row.revDown7 ?? '-', row.revUp30 ?? '-', row.revDown30 ?? '-'].join(' / ')}</td>
                      </tr>
                    {/each}
                  </tbody>
                </table>
